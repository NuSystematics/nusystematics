...
// ---------------------------------------------------------------------------
CCQEXSecCorr::CCQEXSecCorr(YAML::Node const &params)
    : IGENIESystProvider_tool(params),
      ResponseParameterIdx(systtools::kParamUnhandled<size_t>),
      valid_file(nullptr), valid_tree(nullptr) {}

// ---------------------------------------------------------------------------
SystMetaData CCQEXSecCorr::BuildSystMetaData(YAML::Node const &cfg,
                                              paramId_t firstId) {
  ...
  if (ParseYAMLToolConfigurationParameter(cfg, "CCQEXSecCorr", phdr,
                                           firstId)) {
    phdr.systParamId = firstId++;
    smd.push_back(phdr);
  }

  YAML::Node manifest = cfg["CCQEXSecCorr_input_manifest"];
  tool_options["CCQEXSecCorr_input_manifest"] = manifest;

  fill_valid_tree =
      cfg["fill_valid_tree"] ? cfg["fill_valid_tree"].as<bool>() : false;
...
bool CCQEXSecCorr::SetupResponseCalculator(
  ...
  // --- Read manifest ---
  YAML::Node manifest = tool_options["CCQEXSecCorr_input_manifest"];

  std::string dataBaseDir = manifest["DataBaseDir"].as<std::string>();
  std::string filePattern =
      manifest["file_pattern"]
          ? manifest["file_pattern"].as<std::string>()
          : "reweight_data_v3_04_02a_to_v3_06_02_sbn2_CCQE_%FLAVOR%_%ENERGY%_flat.root";
  std::string histName =
      manifest["histogram_name"] ? manifest["histogram_name"].as<std::string>() : "h_weights_map";
  ...
  fEnuMin     = manifest["EnuMin"] ? manifest["EnuMin"].as<double>() : 0.20;
  fEnuMax     = manifest["EnuMax"] ? manifest["EnuMax"].as<double>() : 3.00;
  fEnuSnapTol = manifest["EnuSnapTol"] ? manifest["EnuSnapTol"].as<double>() : 5e-3;

  // Histogram interpretation options
  const bool useNearestBin = manifest["UseNearestBin"] ? manifest["UseNearestBin"].as<bool>() : true;
  const bool edgeClamp     = manifest["EdgeClamp"] ? manifest["EdgeClamp"].as<bool>() : true;
  ...
  // --- Validation tree ---
  fill_valid_tree =
      tool_options["fill_valid_tree"] ? tool_options["fill_valid_tree"].as<bool>() : false;
  if (fill_valid_tree) {
    InitValidTree();
  }
