...
MECq0q3InterpWeighting::SetupResponseCalculator(YAML::Node const &tool_opts)
{
  ...
  if (!manifest)
    throw std::runtime_error("Missing MECResponse_input_manifest");
  ...
  if (manifest["Flavor"] && manifest["Flavors"])
  ...
  if (manifest["Flavors"]) {
    const auto flavors = manifest["Flavors"].as<std::vector<std::string>>();
    if (flavors.empty())
  ...
  } else if (manifest["Flavor"]) {
    const std::string flavor = manifest["Flavor"].as<std::string>();
  ...
  if (manifest["EnergyGrid"])
    defaultEnergyGrid = manifest["EnergyGrid"].as<std::vector<double>>();

  YAML::Node energyGrids;
  const bool haveFlavorEnergyGrids = manifest["EnergyGrids"] ? true : false;
  if (haveFlavorEnergyGrids)
    energyGrids = manifest["EnergyGrids"];
  ...
  auto energy_grid_for = [&](const std::string& flavor) {
    if (!flavor.empty() && haveFlavorEnergyGrids && energyGrids[flavor]) {
      auto grid = energyGrids[flavor].as<std::vector<double>>();
  ...
  const double configuredEnuMin = manifest["EnuMin"] ? manifest["EnuMin"].as<double>() : 0.4;
  const double configuredEnuMax = manifest["EnuMax"] ? manifest["EnuMax"].as<double>() : 2.5;
  ...
  std::string weightMapFile3D =
      manifest["WeightMapFile3D"] ? manifest["WeightMapFile3D"].as<std::string>() : "";
  weightMapFile3D = systtools::expand_env_vars(weightMapFile3D);
  const bool useWeightMap3D = !weightMapFile3D.empty();

  const bool mapIsQ3xQ0 = useWeightMap3D || (manifest["MapIsQ3xQ0"] ? manifest["MapIsQ3xQ0"].as<bool>() : false);
  ...
  std::string dataBaseDir =
      manifest["DataBaseDir"] ? manifest["DataBaseDir"].as<std::string>() : "";
  dataBaseDir = systtools::expand_env_vars(dataBaseDir);

  std::string nuisFlatBaseDir =
      manifest["NuisFlatWeightMapBaseDir"] ? manifest["NuisFlatWeightMapBaseDir"].as<std::string>() : "";
  ...
  const std::string hname_np = manifest["HistNameNP"] ? manifest["HistNameNP"].as<std::string>() : "";
  const std::string hname_nn = manifest["HistNameNN"] ? manifest["HistNameNN"].as<std::string>() : "";
  ...
  if (useWeightMap3D) {
  ...
  } else if (manifest["np_files"] || manifest["nn_files"]) {
    ...
    load_response_data(
        ...
        manifest["np_files"].as<std::vector<std::string>>(),
        manifest["nn_files"].as<std::vector<std::string>>());
