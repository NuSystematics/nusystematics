...
//  flag values
PionAbsWeighter::PionAbsWeighter(YAML::Node const &params)
...
SystMetaData PionAbsWeighter::BuildSystMetaData(YAML::Node const &cfg,
                                              paramId_t id) {
  SystMetaData smd;
...
*/
 //Use descriptors instead of PrettyNames
  ...
  if (!cfg["PionAbs_input_manifest"] ||
      !cfg["PionAbs_input_manifest"].IsMap()) {
    throw invalid_ToolConfigurationYAML()
        << "[ERROR]: When configuring calculated variations for "
           "PionAbs, expected to find a YAML table keyed by "
  ...
  YAML::Node templateManifest = cfg["PionAbs_input_manifest"];
  tool_options["PionAbs_input_manifest"] = templateManifest;

  // Put any options that you want to propagate to the ParamHeaders options
  tool_options["verbosity_level"] =
      cfg["verbosity_level"] ? cfg["verbosity_level"].as<int>() : 0;
...
bool PionAbsWeighter::SetupResponseCalculator(
  ...
  verbosity_level =
      tool_options["verbosity_level"] ? tool_options["verbosity_level"].as<int>() : 0;
  ...
  if (!tool_options["PionAbs_input_manifest"]) {
  ...
  YAML::Node templateManifest =
      tool_options["PionAbs_input_manifest"];
